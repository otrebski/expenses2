...
object SearchPartialEndpoints {
  ...
  private val baseEndpoint = secureEndpoint.in("api" / "expenses" / "search" / "from")
    .in(path[Date]("since").example(Date(2022, 2)))
    .in("to").in(path[Date]("until").example(Date(2022, 5)))
  ...
  val endpoints: List[AnyEndpoint] = List(search, searchNote, searchNotePurpose, searchPurpose, searchPurposeNotes).map(_.endpoint)
