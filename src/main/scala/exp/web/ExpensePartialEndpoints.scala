...
import exp.model.Model.{Date, ExpenseReport, ExpenseToAdd, Note, NotesSuggestionRequest, NotesSuggestionResponse, Purpose}
...
object ExpensePartialEndpoints {
  ...
  val add: PartialServerEndpoint[UsernamePassword, Authentication.User, Model.ExpenseToAdd, RequestError, Model.Expense, Any, IO] = baseExpenseEndpoint
    .put
    .in(jsonBody[Model.ExpenseToAdd].example(ExpenseToAdd(4, Purpose("car"), Note("oil"), Date(2022, 1))))
    .out(jsonBody[Model.Expense].example(Model.Expense(Model.Expense.Id(2), 4, Purpose("car"), Note("oil"), Date(2022, 1))))
