...
object ExpenseService {
  ...
  def mockInstance[F[_]: Applicative]() =
    new ExpenseService[F] {
      ...
      override def find(id: Expense.Id): F[Option[Expense]] =
        data.get(id).pure[F]

      override def add(toAdd: ExpenseToAdd): F[Expense] = {
        val id = Expense.Id(data.size)
        val added =
          Expense(id, toAdd.amount, toAdd.purpose, toAdd.note, toAdd.date)
      ...
      override def purposes(purpose: Purpose): F[List[Purpose]] = {
        val l: List[Purpose] =
          List(Purpose("abcd"), Purpose("asdf"), Purpose("asdcv"))
        l.filter(p => p.contains(purpose))
          //          .map(Purpose(_))
          .pure[F]
      }

      override def purposes(): F[List[Purpose]] =
        List(Purpose("abcd"), Purpose("asdf"), Purpose("asdcv"))
          //        .map(Purpose(_))
          .pure[F]
      ...
      override def summary(since: Date, until: Date): F[List[ExpenseSummary]] =
        data.values
          ...
          .map { case (purpose, amount) =>
            ExpenseSummary(purpose = purpose, amount = amount)
          }
          .toList
          .pure[F]
